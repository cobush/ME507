 ...
 */
void task_ui_pwm (void* p_params)
{
    ...
    // {
      // Prompt the user  
      Serial << "Enter a Direction (1 or 0): "; 

      // one_or_zero is either one or zero
      bool one_or_zero; 

      // set the value input by the user
      one_or_zero= parseIntWithEcho(Serial); 

      // place num in the share duty_cycle
      direction.put(one_or_zero); 
    }
    
}

void task_ui_dir (void* p_params)
{
    (void)p_params;            // Does nothing but shut up a compiler warning
...
void task_out (void* p_params)
{
    ...
    // Calculates the current motor speed and approaches the input value
    for (;;)
    {
...
void setup () 
{
    ...
    pinMode(A3, OUTPUT);
    // Initialize pin A0 as an output pin
    pinMode(A0, OUTPUT);
    
    // Initialize encoder pins from motor encoder
    // ENCA
    pinMode(PA15,INPUT);
    //ENCB
    pinMode(PA14, INPUT);
...
// Create a task which prints a slightly disagreeable message
    xTaskCreate (task_ui_pwm,
                 "User Int. PWM",                 // Name for printouts
                 1536,                            // Stack size
